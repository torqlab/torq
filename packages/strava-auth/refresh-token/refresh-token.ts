 ...
 * @returns {Promise<StravaTokenRefreshResponse>} Promise resolving to parsed token refresh response
 * @throws {Error} Throws AuthError with 'MALFORMED_RESPONSE' code if JSON parsing fails
 * @internal
 */
const parseTokenRefreshResponse = async (response: Response): Promise<StravaTokenRefreshResponse> => {
  try {
    return (await response.json()) as StravaTokenRefreshResponse;
  } catch (error) {
...
const refreshToken = async (
  ...
  const response = await fetchTokenRefreshResponse(STRAVA_TOKEN_URL, body.toString());
  ...
  let tokenData: StravaTokenRefreshResponse;
  try {
